...
# ╔═╡ bb7c93d4-710e-4dfb-a08c-27d7c0aa042c
using Rasters, DataFrames, Statistics

# ╔═╡ 712a3707-7769-47c8-b184-1a9767b75665
using Pipe: @pipe

# ╔═╡ f0a5aab0-29e4-11ef-2b74-c56c7804e545
import ArchGDAL, Random, Images, MLJ

# ╔═╡ d0c4e4d3-8966-4dcf-8bc4-72bd91eab089
import MLJXGBoostInterface

# ╔═╡ 4c5bf774-7295-4c46-97b2-63d2a0b46f4d
import TableOperations

# ╔═╡ 133e3a3a-99f1-44e2-bf50-a7931f58ee8f
const R = Rasters;

# ╔═╡ 6dacec5e-7c36-4dd9-b6bc-45a3e7df6f9b
landcover = let
	water_mask = Raster("water.tif")
	land_mask = Raster("land.tif")
	rebuild(water_mask .+ (2 .* land_mask), missingval=0) |> replace_missing
end;

# ╔═╡ 930f6f54-b1e8-4c53-81bf-b0429a3d2a0b
XGBoostClassifier = MLJ.@load XGBoostClassifier pkg=XGBoost;

# ╔═╡ 7441b517-2ba6-4122-9930-a05f5cd17b6b
features = @pipe RasterStack("2018-01-05_10m.tif", layersfrom=Band, lazy=true) |>
crop(_, to=landcover) |>
read

# ╔═╡ 4a793924-6f2e-4dcd-abf6-3fe32bcc16d9
data = RasterStack(layers(features)..., rebuild(landcover, name=:label))

# ╔═╡ 9ee48a81-bd51-4a81-ad49-c7b8cd3b8a43
mach = let
	traindata = dropmissing(DataFrame(data))
	y, x = MLJ.unpack(traindata, ==(:label))
	y = MLJ.coerce(y, MLJ.Multiclass)

	model = MLJ.FeatureSelector(features=[:X, :Y, :Band], ignore=true) |> XGBoostClassifier()
	mach = MLJ.machine(model, x, y)
	MLJ.fit!(mach)
end

# ╔═╡ 877989d4-dea7-4506-9430-dd5415e2e5e7
let
	ŷ = @pipe MLJ.predict_mode(mach, features) |> 
	MLJ.int |> 
	UInt8.(_) |>
	Raster(_, dims(features))

	Rasters.write("mask.tif", ŷ, force=true)

	#@pipe (ŷ.data ./ 2.0f0) |> permutedims(_, (2, 1)) |> Images.Gray.(_)
end

# ╔═╡ 20e3a0c9-a6a6-4718-875e-b7ba909de934
let
	fs = RasterStack("2018-01-05_10m.tif", layersfrom=Band, lazy=true)[1:5120,1:5120]
	g = fs[:B3]
	nir = fs[:B8]

	ndwi = (g .- nir) ./ (g .+ nir)
	Rasters.write("ndwi.tif", ndwi, force=true)
end

# ╔═╡ 51ed5d7e-2181-4110-a2ba-01404ed846d7
let
	water = Raster("ndwi.tif", lazy=true) .> -0.15
	water = Rasters.read(water) .|> UInt8
	Rasters.write("water_mask.tif", water, force=true)
end

# ╔═╡ 8ab51153-fbc9-4148-8f52-cb1ff42e4e24
let
	fs = RasterStack("2018-01-05_20m.tif", layersfrom=Band, lazy=true)
	swir = fs[:B12]
	nir = fs[:B8A]

	ndbi = (swir .- nir) ./ (swir .+ nir)
	Rasters.write("ndbi.tif", ndbi, force=true)
end

# ╔═╡ ac1e2791-3f92-4f11-b8c4-782b6ba169b1
let
	built = Raster("ndbi.tif", lazy=true) .> -0.2
	built = Rasters.read(built) .|> UInt8
	Rasters.write("built_mask.tif", built, force=true)
end

# ╔═╡ a3f9f808-a4ef-4c99-b1c8-91acefefcb38
...
# ╔═╡ e1e241dc-969d-4df0-b2c9-f6022e4994a1
Base.length(x::TileSampler) = length(x.tiles);

# ╔═╡ 398a6a0e-09fb-4eb0-b9da-91522ed410ec
...
# ╔═╡ d194b6af-d0f0-48f1-84e9-0bef68c8220e
struct JoinedSampler{T}
	samplers::Vector{T}
	indices::Vector{Int}

	function JoinedSampler(xs::Vararg{T}) where {T}
		indices = map(length, xs) |> cumsum |> collect
		return new{T}(collect(xs), indices)
	end

	function JoinedSampler(xs...)
		throw(ArgumentError("Samplers must be of the same type!"))
	end
end;

# ╔═╡ d8d820a1-e53d-455a-84f6-21725c47d1cb
Base.length(x::JoinedSampler) = last(x.indices);

# ╔═╡ 8305a4d7-2b07-4556-98f1-44612c81ef08
function Base.getindex(x::JoinedSampler, i::Int)
	j = findfirst(>=(i), x.indices)
	offset = j == 1 ? 0 : x.indices[j-1]
	return x.samplers[j][i-offset]
end;

# ╔═╡ 8b8cb238-f9ff-40e9-acf2-2941c219eabf
function Base.getindex(x::JoinedSampler, i::AbstractVector)
	return map(j -> Base.getindex(x, j), i)
end;

# ╔═╡ 573fda7b-d51d-49e8-930e-1335ced6ae5c
function Base.getindex(x::TileSampler, i::Int)
	return Base.getindex(x, [i]) |> first
end;

# ╔═╡ 6628167a-102a-43ac-ac64-48f711a4e755
md"""
# Tile Sampler
"""

# ╔═╡ 6a0d8585-f3c9-4225-8929-cf3154feb543
import MLUtils

# ╔═╡ f07e5df0-9971-4840-b8f8-be348185ad89
function unpack(stack::AbstractRasterStack, layers...)
	return unpack([stack], layers...)
	@pipe map(x -> x isa AbstractVector ? Tuple(x) : x, layers) |>
	map(x -> Raster(stack[x]), _)
end;

# ╔═╡ bba8961e-50a6-4ec6-bf83-3a5898b1e93a
function unpack(stacks::AbstractVector, layers...)
	map(layers) do layer
		layer = layer isa AbstractVector ? Tuple(layer) : layer
		@pipe map(x -> Raster(x[layer]).data, stacks) |> cat(_..., dims=4)
	end
end;

# ╔═╡ ade27409-2eb8-46db-a351-6af817d5bc30
md"""
# From Table
"""

# ╔═╡ 1427efed-1796-4239-bac7-1e26bbbac3f3
...
# ╔═╡ 401a68ae-9272-425f-9978-5d9d0614f9bb
...
# ╔═╡ a3fbfd01-ac9c-49de-8e17-2ed605ba95e5
function _coords_to_ords(coords::AbstractVector, dim::Rasters.Dimension)
	_coords_to_ords(coords, collect(dim))
end;

# ╔═╡ 4b582aca-59ec-40e2-93c1-b891d1dd4f26
function _coords_to_ords(coords::Tuple, dims::Tuple)
	@assert length(coords) == length(dims) "Coords and Dims Mismatch!"
	Tuple(_coords_to_ords(c, d) for (c, d) in zip(coords, dims))
end;

# ╔═╡ ee45cd95-0cf5-4f09-a980-137011ec6581
...
# ╔═╡ 5d7f8078-262a-4782-aaa0-23e34faad0b7
function _dim_cols(table, dims::Tuple)
	dim_names = R.name.(dims)
	if :geometry in Tables.columnnames(table)
		coords = Tables.getcolumn(table, :geometry)
		return Tuple(map(x->x[i], coords) for i in eachindex(dim_names))
	else
		return Tuple(Tables.getcolumn(table, col) for col in dim_names)
	end
end;

# ╔═╡ 0fc179bc-4dea-4817-8c1e-c144998588e4
function _data_cols(table, dims, cols=nothing)
	if isnothing(cols)
		dim_names = R.name.(dims)
		col_names = Tables.columnnames(table)
		data_cols = filter(col -> !(col in vcat(dim_names, :geometry)), col_names)
		return Tuple(Tables.getcolumn(table, col) for col in data_cols)
	else
		return Tuple(Tables.getcolumn(table, col) for col in cols)
	end
end;

# ╔═╡ 41a88d88-5f79-455c-802f-d73e84e294e8
function _data_col_names(table, dims)
	dim_names = R.name.(dims)
	col_names = Tables.columnnames(table)
	return filter(col -> !((col in dim_names) || (col == :geometry)), col_names)
end;

# ╔═╡ 4e16d015-2bc9-4134-ae7d-bb5343d677cf
function _check_table_dims(table, dims)
	if :geometry in Tables.columnnames(table)
		geoms = Tables.getcolumn(table, :geometry)
		@assert eltype(geoms) <: Tuple ":geometry column must contain tuples!"
		@assert fieldcount(eltype(geoms)) == length(dims) "Number of dims mismatch"
	else
		for dim in R.name.(dims)
			@assert dim in Tables.columnnames(table) "Dimension $dim not found!"
		end
	end
end;

# ╔═╡ ce27b5a4-2c06-41a0-8e5e-abd5ae8fc497
function _check_rows(table, dims)
	nrows = Tables.rowcount(table)
	max_rows = reduce(*, map(length, dims))
	@assert nrows <= max_rows "Table rows exceed size of Raster!"
end;

# ╔═╡ fd8aac41-763c-4605-8113-cbafb148e757
function _check_table_cols(table, cols)
	table_cols = Tables.columnnames(table)
	@assert !isempty(cols) "Data columns cannot be empty!"
	for col in cols
		@assert col in table_cols "Column $col does not exist!"
	end
end;

# ╔═╡ 8c7af5f6-0119-47be-ad99-37d91d5c91c6
function from_table(table, dims; cols=nothing, missingval=missing)
	# Calculate Indices from Coords
	_check_rows(table, dims)
	_check_table_dims(table, dims)
	dim_cols = _dim_cols(table, dims)
	ords = _coords_to_ords(dim_cols, dims)
	indices = _ords_to_indices(ords, dims)
	#sorted_indices = sortperm(indices)

	# Determine Data Columns 
	cols = isnothing(cols) ? _data_col_names(table, dims) : cols
	_check_table_cols(table, cols)
	data_cols = _data_cols(table, dims, cols)

	# Restore Rasters
	rasters = Any[]
	for (col, data) in zip(cols, data_cols)

		# Create Destination Array
		dst_size = reduce(*, length.(dims))
		data_dst = Vector{eltype(data)}(undef, dst_size)

		# Write Non-Missing Values to Destination
		data_dst[indices] .= data

		# Restore Dimensions
		#data_dst = data[sorted_indices]
		raster = R.Raster(data_dst, dims, name=col, missingval=missingval)
		push!(rasters, raster)

		# Mask Missing Values
		#mask_dst = zeros(Bool, dst_size)
		#mask_dst[indices] .= true
		#m = R.Raster(mask_dst, dims, missingval=false)
		#push!(rasters, R.mask(raster, with=m))
	end
	
	return length(data_cols) > 1 ? R.RasterStack(rasters...) : first(rasters)
end;

# ╔═╡ ffe218da-897a-40e7-a119-5fcae15efef4
# ╠═╡ disabled = true
#=╠═╡
let
	fs = RasterStack("2018-01-05_10m.tif", layersfrom=Band, lazy=true)[1:5120,1:5120]
	...
	coords = [(x, y) for x in 1:256:5120 for y in 1:256:5120]
	tiles = [(@view fs[X(x:x+255), Y(y:y+255)]) for (x, y) in coords]

	ŷ = Any[]
	for tile in tiles
		@pipe read(tile) |>
		MLJ.predict_mode(mach, _) |> 
		MLJ.int |> 
		UInt8.(_) |>
		Raster(_, dims(tile)) |>
		push!(ŷ, _)
	...
	data = @pipe map(DataFrame, ŷ) |> 
	reduce(vcat, _) |> 
	from_table(_, dims(fs))
	#@time data = from_tables(ŷ, dims(fs))

	#data = @pipe map(x -> replace_missing(x, UInt8(0)), ŷ) |> 
	#extend(_, to=fs) |> 
	#reduce(+, _) #|>
	#Rasters.write("mask.tif", _, force=true)

	ismissing.(data.data) |> sum |> println

	Rasters.write("mask.tif", data, force=true)
end
  ╠═╡ =#

# ╔═╡ 6a0418b7-1e81-4eda-ab89-234fcf9ebd81
let
	# Create Raster
	_dims = (X(LinRange(0.0, 1.5, 15)), Y(LinRange(1.5, 0.0, 15)), Band([:B2, :B3, :B4]))
	r = Raster(rand(15, 15, 3), _dims, missingval=0)

	# Mask Values
	m = Raster(rand([0,1], 15, 15, 3), dims(r))
	r = replace_missing(r .* m)

	# Create Table and Shuffle Rows
	t1 = Tables.columntable(r)
	t2 = Tables.rowtable(t1) |> Random.shuffle |> Tables.columntable
	
	restored_1 = from_table(t1, dims(r))
	#restored_2 = from_table(t2, dims(r))

	all(r .=== restored_1) |> println
	#all(r .=== restored_2) |> println
end

# ╔═╡ fd9235c2-5048-4f1e-9ee9-73141da3a11b
let
	#t = DataFrame(a=rand(3), b=rand(3))
	#Tables.columntable(t)[[:a, :b]]

	_dims = (X(LinRange(0.0, 1.5, 15)), Y(LinRange(1.5, 0.0, 15)), Band([:B2, :B3, :B4]))
	r = Raster(rand(15, 15, 3), _dims, missingval=0)
	t = DimTable(r, mergedims=(:X,:Y)=>:geometry, layersfrom=Band) |> DataFrame
	t2 = DataFrame(r)

	#all(Raster(from_table(t, _dims[1:3])) .== r)
	all(from_table(t2, _dims) .== r)

	#from_table(t2, _dims[1:2])
	@pipe DataFrames.groupby(t2, [:X, :Y]) |> 
	DataFrames.combine(first, _) |>
	from_table(_, _dims[1:2])
	#Datgroupby(t2, )
end

# ╔═╡ d53b075f-9faa-48d9-97cf-db7a7cc58047
let
	r1 = Raster(rand(3, 3), (X(1:3), Y(3:-1:1)), missingval=0.0)
	m = Raster(rand(Bool, 3, 3), (X(1:3), Y(3:-1:1)), missingval=false)

	r2 = mask(r1, with=m)
	r3 = replace_missing(r2)

	# Restore with all Values
	for r in (r1, r2, r3)
		t1 = DataFrame(r)
		t2 = DataFrame(r) |> Random.shuffle!
		all(from_table(t1, dims(r)) .=== r) |> println
		all(from_table(t2, dims(r)) .=== r) |> println
	end

	#t = DataFrame(r1)[1:2:9,:]
	#from_table(t, dims(r1))
	#rasterize(t, fill=:layer1, reducer=first)
end

# ╔═╡ a2556cff-86af-4736-abb3-af8a0cd7bc4c
let
	r = Raster(rand(UInt8, 20000, 20000), (X(1:20000), Y(20000:-1:1)), missingval=0.0)
	t = DataFrame(r) #|> Random.shuffle!
	@time from_table(t, dims(r))
end

# ╔═╡ 41abe2be-de3b-44f6-a8cf-81f6e2b17b8d
let
	sampler1 = TileSampler(features, 250, 125)
	sampler2 = TileSampler(features, 250, 125)
	sampler = JoinedSampler(sampler1, sampler2)

	@pipe MLUtils.mapobs(x -> unpack(x, [:B2, :B3, :B4], :B8), sampler) |>
	MLUtils.shuffleobs(_) |>
	MLUtils.DataLoader(_, batchsize=4)

	restored = @pipe MLUtils.shuffleobs(sampler) |> 
	map(DataFrame, _) |> 
	reduce(vcat, _) |>
	transform!(_, [:X, :Y] .=> ByRow(x -> round(x, digits=6)), renamecols=false) |>
	DataFrames.groupby(_, [:X, :Y]) |>
	DataFrames.combine(_, DataFrames.Not([:X, :Y]) .=> mean, renamecols=false) |>
	from_table(_, dims(features))

	all(Raster(restored) .≈ Raster(features))
	#(sampler1, sampler2) |> collect
	#MLUtils.joinobs(sampler1, sampler2) |> typeof
end

# ╔═╡ de3e8cf6-e7a0-46fc-84e4-f9c09b4b1926
_find_non_missing(vals, missingval::Missing) = findall(!ismissing, vals);

# ╔═╡ 14e7fe1d-97af-4645-9747-eb0d92629af0
_find_non_missing(vals, missingval::Nothing) = findall(!isnothing, vals);

# ╔═╡ 477a160b-2ca8-4c45-b621-9464d611859d
_find_non_missing(vals, missingval) = findall(!=(missingval), vals);
...
manifest_format = "2.0"
project_hash = "e886756445cedc57ec9190277fd42aea84a63199"
...
[[deps.Rasters]]
deps = ["Adapt", "ColorTypes", "CommonDataModel", "ConstructionBase", "Dates", "DimensionalData", "DiskArrays", "Extents", "FillArrays", "Flatten", "GeoFormatTypes", "GeoInterface", "Missings", "Mmap", "OffsetArrays", "ProgressMeter", "RecipesBase", "Reexport", "Setfield"]
git-tree-sha1 = "729333d913ff47ea23a47b5fc753354c630e666a"
