...
# ╔═╡ 852a7bd0-0dae-474a-9b1c-cc80db32f9ef
using Images, WGLMakie

# ╔═╡ 10e3fc84-c57d-4498-b394-0d96ecbd4d0e
using Pipe: @pipe

# ╔═╡ fe679e14-2468-11ef-28ab-f5bb7bc7fb4b
import ArchGDAL, Rasters

# ╔═╡ c2c6b4f7-c2bb-4b92-944b-7bb2b82822e1
import PlutoUI

# ╔═╡ 07f63de1-1f61-4a1a-9689-8eff06c0d6d8
const R = Rasters;

# ╔═╡ 845c3d87-c9a7-4e43-93d3-68bcf4d4eaa6
for f in (:layermetadata, :layerdims)
    @eval (R.DD.$f)(x::BandSet) = parent(x) |> R.DD.$f
end

# ╔═╡ 22915dac-2681-4d55-8adb-07003067572f
for f in (:missingval, :dims, :refdims, :metadata)
    @eval (R.$f)(x::BandSet) = parent(x) |> R.$f
end

# ╔═╡ 9ba630b4-b02b-4983-9ae4-44553c23bf4c
function list_files(dir::String)
    vcat([map(x -> joinpath(root, x), files) for (root, _, files) in walkdir(dir)]...)
end;

# ╔═╡ 4d1a14d3-7041-4265-8e5c-cc176a0d262c
function read_bands(dir::String, pattern::Regex; kwargs...)
	files = @pipe list_files(dir) |> filter(x -> contains(x, pattern), _)
	names = map(x -> match(pattern, x), files) .|> first .|> Symbol
	R.RasterStack(
		[R.Raster(f, name=n; kwargs...) for (f, n) in zip(files, names)]...)
end;

# ╔═╡ 64fd873a-a986-4d06-a973-6fd32c1e5234
begin
struct BandSet{S,K,T,L,D,R,LD,M,LM,MV} <: Rasters.AbstractRasterStack{K,T,2,L}
    stack::Rasters.RasterStack{K,T,2,L,D,R,LD,M,LM,MV}

	function BandSet(rs::Rasters.RasterStack{K,T,2,L,D,R,LD,M,LM,MV}, S::Symbol) where{K,T,L,D,R,LD,M,LM,MV}

		return new{S,K,T,L,D,R,LD,M,LM,MV}(rs)
	end
end

function BandSet(dir::String, pattern::Regex, S::Symbol; kwargs...)
	return BandSet(read_bands(dir, pattern; kwargs...), S)
end
end;

# ╔═╡ 9158a256-db4c-43b8-b9fe-81184f64dc78
Base.parent(x::BandSet) = getfield(x, :stack);

# ╔═╡ 270642db-43c8-41e0-beb6-044499908db2
Base.nameof(::Type{<:BandSet{S}}) where {S} = "BandSet{:$S}"

# ╔═╡ dddb800f-ee44-469c-a3de-d2e585319bbd
Base.getindex(s::BandSet, name::String) = parent(s)[Symbol(name)]

# ╔═╡ 0bdd61fc-deb5-4599-a92c-0dfed6adaa59
Base.getindex(s::BandSet, name::Symbol) = parent(s)[name]

# ╔═╡ c72e4b9e-adae-4d0a-96b4-df476f4d53c7
Base.getindex(s::BandSet{S}, i::Union{Tuple{R.DD.Dimensions.Dimension, Vararg{R.DD.Dimensions.Dimension}}, AbstractArray{<:R.DD.Dimensions.Dimension}, AbstractArray{<:Tuple{R.DD.Dimensions.Dimension, Vararg{R.DD.Dimensions.Dimension}}}, R.DD.DimIndices, R.DD.DimSelectors, R.DD.Dimensions.Dimension}) where {S} = BandSet(parent(s)[i], S)

# ╔═╡ 66940b78-435b-4399-ab93-f3927660aa04
bs = let
	scene = "S2B_MSIL2A_20200804T183919_N0214_R070_T11UPT_20200804T230343"
	bs = BandSet(scene, r"(B[08][\dA])_20m\.jp2$", :Sentinel2, lazy=false)
end

# ╔═╡ 1bf18840-62fb-4bfc-82ee-486e4d99eaee
md"""
**Gray:** $(@bind gray PlutoUI.Select([:B02, :B03, :B04, :B8A]; default=:B8A))

**RGB:** $(@bind r PlutoUI.Select([:B02, :B03, :B04, :B8A]; default=:B04))
$(@bind g PlutoUI.Select([:B02, :B03, :B04, :B8A]; default=:B03))
$(@bind b PlutoUI.Select([:B02, :B03, :B04, :B8A]; default=:B02))

**Bounds:** $(@bind lb PlutoUI.NumberField(0:0.01:1.0, default=0.02))
$(@bind ub PlutoUI.NumberField(0:0.01:1.0, default=0.98))
"""

# ╔═╡ 5c74031e-fdc8-43ed-9d70-f2ac070f8caf
@time visualize(
	bs[r].data, 
	bs[g].data,
	bs[b].data, 
...
# ╔═╡ 9be90caf-1efd-482d-9559-c846cbcad476
function visualize(x; lower=0.02, upper=0.98)
	@pipe linear_stretch(x, lower, upper) |>
	permutedims(_, (2, 1)) |>
	colorview(Gray, _)
end;

# ╔═╡ b6f5e490-af8f-4881-95bd-db4320c88626
...
# ╔═╡ b2bc8322-e52b-46c9-8877-95187b842149
...
# ╔═╡ 9b302863-090b-4959-885d-e59939f9600d
...
# ╔═╡ 099d343a-256b-4024-96ca-59a169cf4c35
function plot_hist!(ax, raster, label; lower_bound=0.02, upper_bound=0.98)
	edges, counts = build_histogram(raster, 1000)
	lb = @pipe findfirst(<=(lower_bound), cumsum(counts) ./ sum(counts)) |> edges[_]
	ub = @pipe findfirst(>=(upper_bound), cumsum(counts) ./ sum(counts)) |> edges[_]
	
	vals = filter(x -> lb <= x <= ub, vec(raster.data))
	#edges, counts = build_histogram(vals, 1000)

	#GLMakie.hist
	GLMakie.stephist!(ax, vals, bins=100, linewidth=2, label=label)
	#GLMakie.lines!(ax, edges[1:10:1000], counts[1:10:1000], linewidth=1, label=label)
end;

# ╔═╡ 2587808e-228c-4e48-b984-9a777206a7e3
let
	edges, counts = build_histogram(bs[:B02], 1000)
	ub = @pipe findfirst(>=(0.98), cumsum(counts) ./ sum(counts)) |> edges[_]
	lb = @pipe findfirst(<=(0.02), cumsum(counts) ./ sum(counts)) |> edges[_]
	vals = filter(x -> lb <= x <= ub, vec(bs[:B02].data))
	#fig, ax, plt = GLMakie.barplot(edges, counts, gap=0.0, width=10)

	fig = GLMakie.Figure(size=(1000,500))
	ax = GLMakie.Axis(fig[1,1])
	for band in (:B02, :B03, :B04, :B8A)
		plot_hist!(ax, bs[band], string(band))
	end

	Legend(fig[1,2], ax)
	
	#ax, plt = GLMakie.hist(fig[1,1], vals, bins=100)
	fig
end

# ╔═╡ 74d544c8-f9fa-4c17-b740-24ab88e6d6b1
function linear_stretch(img, lower, upper)
	# Build Histogram
	edges, counts = build_histogram(img, 10000)

	# Get Percentiles
	ps = cumsum(counts) ./ sum(counts)

	# Get Lower and Upper Bounds
	lb = @pipe findfirst(>=(lower), ps) |> clamp(_, 1, 10000) |> edges[_] |> Float16
	ub = @pipe findfirst(>=(upper), ps) |> clamp(_, 1, 10000) |> edges[_] |> Float16

	# Stretch Image
	return clamp!((img .- lb) ./ (ub .- lb), Float16(0.0), Float16(1.0))
end;

# ╔═╡ 958a98d5-ef22-4b15-8344-3d8ead388c18
function visualize(r, g, b; lower=0.02, upper=0.98)
	dst = zeros(N0f8, 3, size(r, 2), size(r, 1))
	for (i, x) in enumerate((r, g, b))
		d = @view dst[i,:,:]
		normalized = linear_stretch(x, lower, upper)
		permutedims!(d, normalized, (2, 1))
	end
	return colorview(RGB, dst)
end;

# ╔═╡ 5f566c9b-2a62-4d59-acac-88288f330f22
let
	fig = Figure(size=(500,500))
	menu = Menu(fig, options = ["B02", "B03", "B04"], default = "B02")

	b = Observable{Symbol}(:B02)
	...
	#menu
	image(fig[1,2], Images.imresize(visualize(bs[b.val]), ratio=0.1))
	fig
	#img = 
	#fig
end
...
manifest_format = "2.0"
project_hash = "5fbfe37b07018b41d311a34f7fbb0dc2cc9d3a41"
